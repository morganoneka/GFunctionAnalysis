...
for (inx in phenotypes){
  ...
  filename = paste(paste(ref_phenotypes, nonref_phenotypes, sep="_VS_"), "_KM", ".csv", sep="")
  ...
  for (file in list.files(path=input, pattern=paste("*", filetype, sep="."), full.names=TRUE, recursive=FALSE)){
    ...
    XYdata <- XYdata[!duplicated(XYdata[,c(xcol,ycol)]),]
    ...
    # replace any commas in string 
    file_safe <- str_replace(file, ",", "_")
    if (sum(unlist(XYdata$PastedPhenotype) == "Reference") < 1 | sum(unlist(XYdata$PastedPhenotype) == "Non-Reference") < 1) {
      gcross_results = rbind.data.frame(gcross_results, c(file_safe, rep(0, radiusmax+1)), stringsAsFactors = FALSE)
    } else{
      Gcross1 = Gcross(pp,"Reference","Non-Reference",r = 0:radiusmax)
      gcross_results = rbind.data.frame(gcross_results, c(file_safe, t(Gcross1$km)), stringsAsFactors = FALSE)
    }
  }
